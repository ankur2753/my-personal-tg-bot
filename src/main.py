...
async def handle_welcome(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not settings.is_user_allowed(user_id):
        ...
        await update.message.reply_text(
            f"⛔ *Unauthorized Access*\n\nYour Telegram User ID (`{user_id}`) is not whitelisted in `appsettings.json`.",
            parse_mode="Markdown"
        )
        return
...
async def handle_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_msg = (
        ...
        f"• *Active User Whitelist:* `{settings.allowed_telegram_user_ids}`\n"
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not settings.is_user_allowed(user_id):
        ...
        await update.message.reply_text(f"⛔ Unauthorized user (`{user_id}`).", parse_mode="Markdown")
        return

    intent = router.detect_intent(text)
    logger.info(f"Received message from User {user_id}: '{text}' -> Classified Intent: {intent}")

    if intent == "welcome" or intent == "default":
